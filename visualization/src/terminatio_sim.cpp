...
ros::NodeHandle n;
  ...
  pose.request.pose.header.frame_id="base_link";
  pose.request.pose.pose=feedback->pose;

  tf::quaternionMsgToTF(pose.request.pose.pose.orientation,q);
  qn=tf::Quaternion(1,0,0,0);
  qn=((q*qn)*q.inverse());
  offset=tf::Vector3(qn.x(),qn.y(),qn.z());
  offset.normalize();
  offset*=-0.10;
  pose.request.pose.pose.position.x+=offset.x();
  pose.request.pose.pose.position.y+=offset.y();
  pose.request.pose.pose.position.z+=offset.z();
  pose.request.pose.header.stamp= ros::Time::now();
  if (client.call(pose))
  {
ROS_INFO("OK");
	if(pose.response.result==true)ROS_INFO("I'll follow!");
	else ROS_WARN("I can't reach!");
...
int main(int argc, char** argv)
{
 ...
 // ros::Timer frame_timer = n.createTimer(ros::Duration(0.01), frameCallback);
  ...
  position = tf::Vector3( 0.40, 0, 0.4);
  make6DofMarker( false, visualization_msgs::InteractiveMarkerControl::NONE, position, true );
  //position = tf::Vector3( 3, 3, 0);
